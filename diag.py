...
def main() -> None:
    ...
    if resp.status_code == 200:
        ...
        if resp.status_code == 500 and "InternalError" in (resp.text or ""):
            print(
                "\n=> Raw REST returned 500 InternalError on the blob GET.\n"
                "   Note: the x-ms-request-id above is generated by AZURE for this response --\n"
                "   it is not something we send, so it can't be reused or hardcoded to change\n"
                "   the outcome. Probing whether the ACCOUNT is healthy (metadata ops use a\n"
                "   different code path than reading blob DATA)..."
            )
            # If these list ops succeed while the GET 500s, the account is UP and the problem is
            # specific to reading THIS blob (bad object, tiering, or a storage backend issue).
            # If these ALSO 500, the whole account is unhealthy -> infra ticket + x-ms-request-id.
            list_containers(account, token)
            list_blobs(account, container, token, prefix=blob.split("/")[0] if "/" in blob else "")
            print(
                "\n   Interpretation:\n"
